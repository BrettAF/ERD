...
def funDenseLSTM(my_training_generator,window_size,number_of_channels,my_validation_generator):
  ...
  DenseLSTM = keras.Sequential([
      layers.Reshape((145,36),input_shape=input_shape),
      layers.Dense(145),
      layers.Dense(145),
...
def print_report(my_testing_generator,m2Dlstm,m1DLSTM,m2Dconv):
  ...
  with open('results_'+curr_time+'.txt',"w") as writer:
    ...
    print('m2Dlstm')
    test_loss, test_acc = m2Dlstm.evaluate(my_testing_generator, verbose=2)
    print( test_acc)
    writer.write(str('m2Dlstm:\t test loss:'+str(test_loss)+ ", test acc:"+str(test_acc)))

    print('m1DLSTM')
    test_loss, test_acc = m1DLSTM.evaluate(my_testing_generator, verbose=2)
    print( test_acc)
    writer.write('m1DLSTM:\t test loss:'+str(test_loss)+ ", test acc:"+str(test_acc))

    print('m2Dconv')
    test_loss, test_acc = m2Dconv.evaluate(my_testing_generator, verbose=2)
    print( test_acc)
...
class My_Custom_Generator(keras.utils.Sequence) :
  ...
  def __getitem__(self, idx) :
    ...
    return np.array([np.memmap(str(file_name), dtype='float32', mode='r', shape=(290,18))
...
bigConv.fit(my_training_generator,
                epochs = number_of_epochs,
                verbose = 1,
                validation_data=my_validation_generator)
bigConv.save("BigConv")