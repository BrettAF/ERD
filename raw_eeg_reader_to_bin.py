# -*- coding: utf-8 -*-
"""Raw EEG Reader to big labeled bin
...
def convertFileToNP(file_name, recording_start_time):
  ...
  with open(file_name+'.csv','r') as csvinput:
      ...
      for row in reader:
        ...
        row_to_keep=[]
        for column in columns_to_keep:
            row_to_keep.append(float(row[column]))

        row_array = np.array(row_to_keep)
        if index%100000==5:
          print(index)
        if index%600==0:
          label_array.append(int(assignLabel( index*.003906 + recording_start_time, seizure_start_times, seizure_end_times ) ))
        dataset[index-1]=row_array
        dataset.flush()
...
for folder in folders:
  ...
  for i in range(0,1):
    ...
    row_count,labels=convertFileToNP(eeg_files[i],file_start_times[i])

    np.array(labels)
    np.save(eeg_files[i]+'labels',labels)

#path='/content/drive/MyDrive/ERD'
#os.chdir(path)
#eeg_files, file_start_times, seizure_start_times, seizure_end_times,subject_end_time = summaryReader('chb01')

#convertToCsv('chb01_01')
#row_count=convertFileToNP('chb01_01',file_start_times[0])
#print('chb01_01',row_count)

#file='chb01_01'
#convertToCsv(file)

#row_count, label_array=convertFileToNP(file,file_start_times[2])
#print(file,row_count)

#adataset = np.memmap(file+'formated', dtype='float32', mode='r', shape=(row_count,18))
#os.remove(file+'formated')
#newdataset=np.memmap(file+'formated', dtype='float32', mode='w+', shape=(len(label_array)*600,18))
#newdataset=adataset[0:len(label_array)*600]

#new_dataset = np.memmap('chb01_03l', dtype='float32', mode='r', shape=(1536,600,18))
