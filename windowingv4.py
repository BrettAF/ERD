...
Original file is located at
    https://colab.research.google.com/drive/10Znw7zsMAm8pPT4MCnbTKzZvoDcX_sdZ
...
def FFT(dfs,labels):
  for df in range(0, dfs.shape[0]):
    dfs[df]=np.real(fft(dfs[df]))
  print('FFT\t',dfs.shape,',',len(labels),',',dfs[2][1][3])
  return dfs,labels
...
def balance(dfs,labels):
  ...
  under=RandomUnderSampler(sampling_strategy=.90)# An undersampling strategy

  nr = NearMiss()#an undersampling strategy
  over=SMOTE(sampling_strategy=.70)  #an oversampling strategy

  if ((counter[1])/len(dfs))<=.30:
    dfs=dfs.reshape(-1,600*18)
    dfs, labels = nr.fit_sample(dfs, labels, sampling_strategy=.35)
...
folders_for_making_model = ('chb01',)#,'chb03','chb04','chb05','chb06','chb07','chb08','chb09','chb10','chb11','chb12','chb13','chb14','chb15','chb16','chb17','chb18','chb19','chb20','chb21','chb22','chb23','chb23','chb24')
