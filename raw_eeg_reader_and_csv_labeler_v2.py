...
def summaryReader(folder):
  ...
  for line in summaryfile:
    ...
    ss_search=re.match("Seizure \d* Start Time: (\d+)",line)
    if ss_search:
      print(ss_search.group(1))
      seizure_start_times.append(time_since_recording_began + int(ss_search.group(1)))
    #This searches for seizure end
    se_search = re.match("Seizure \d* End Time: (\d+)",line)
    if se_search:
...
os.chdir('..')
eeg_files, file_start_times, seizure_start_times, seizure_end_times = summaryReader('chb06')
print(seizure_start_times)
print(seizure_end_times)